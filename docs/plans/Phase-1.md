...
- **Unity 6.0 upgrade COMPLETE** (all phases 0-7 from `upgrade-unity-6` branch)
- **All 52+ tests passing** in Unity 6
- Python 3.10.12 installed (verify with `python --version`)
- Unity 6.0 (6000.0.x) project opens without errors
...
**Goal:** Set up Python ML-Agents 2.0.x and Unity Sentis 2.1 for reinforcement learning training in Unity 6.

**Files to Modify/Create:**
- Python environment in project root
- `Packages/manifest.json` - Add ML-Agents and Sentis Unity packages
- `docs/ML_AGENTS_SETUP.md` - Setup documentation

**Prerequisites:**
- Python 3.10.12 installed (verify: `python --version`)
- conda/mamba recommended (better Python version control than uv)
- Unity 6.0 project validated and all tests passing

**Implementation Steps:**

1. **Verify Unity 6 Upgrade Completion**:
   - Confirm Unity 6.0 (6000.0.x) project opens without errors
   - Run test suite: Window > General > Test Runner
   - Verify all 52+ tests are passing
   - Check CharacterMovement, CharacterCombat, CharacterHealth systems work
   - If any tests fail, STOP and complete Unity 6 upgrade first

2. **Create Python 3.10.12 virtual environment**:
   - **Recommended**: Use conda for precise Python version control
     ```bash
     conda create -n mlagents python=3.10.12
     conda activate mlagents
     ```
   - **Alternative**: Use uv (if it supports Python 3.10.12)
     ```bash
     uv venv ml-agents-env --python 3.10.12
     source ml-agents-env/bin/activate
     ```
   - Verify: `python --version` (MUST show 3.10.x, NOT 3.11 or 3.12)
   - Document activation command for future use

3. **Install ML-Agents 2.0.x Python package**:
   - Install ML-Agents: `pip install mlagents`
   - Verify installation: `mlagents-learn --help`
   - Check versions:
     ```bash
     python -c "import torch; print(f'PyTorch: {torch.__version__}')"
     pip list | grep mlagents
     ```
   - Should show PyTorch 2.1.1 or 2.2.1
   - **Windows GPU users**: Install PyTorch separately first:
     ```bash
     pip install torch~=2.2.1 --index-url https://download.pytorch.org/whl/cu121
     pip install mlagents
     ```
   - Document exact versions: `pip list > ml-agents-versions.txt`

4. **Add ML-Agents 2.0.x Unity package**:
   - Open Unity Package Manager (Window > Package Manager)
   - Click "+" dropdown > "Add package by name..."
   - Enter: `com.unity.ml-agents`
   - Unity 6 will show version 2.0.1 or 2.0.2 (this is correct!)
   - **Do NOT use 3.0.0 or 4.0.0** - they have Unity 6 compatibility issues
   - Wait for package import (may take several minutes)

5. **Add Unity Sentis 2.1 package (REQUIRED)**:
   - In Package Manager, click "+" > "Add package by name..."
   - Enter: `com.unity.sentis`
   - Select version 2.1.1 or latest 2.x
   - **Critical**: Sentis is REQUIRED for ML-Agents inference
   - Sentis replaces deprecated Barracuda
   - Wait for package import

6. **Verify Unity 6 + ML-Agents 2.0 + Sentis setup**:
   - Check `Unity.MLAgents` namespace available in C# scripts
   - Check `Unity.Sentis` namespace available
   - Verify no console errors related to ML-Agents or Sentis
   - Unity 6 should automatically use compatible scripting settings
   - Check API Compatibility Level: Edit > Project Settings > Player > ".NET Standard 2.1"

7. **Create documentation file** `docs/ML_AGENTS_SETUP.md`:
   - Document Python 3.10.12 installation (conda recommended)
   - List exact package versions (mlagents, PyTorch, Unity packages)
   - Note Unity 6.0 compatibility: ML-Agents 2.0.x + Sentis 2.1
   - Document that Barracuda is deprecated, Sentis is replacement
   - Include troubleshooting for common Unity 6 + ML-Agents issues
   - Provide quick start command for training

**Verification Checklist:**
- [ ] Unity 6.0 upgrade verified complete (all tests passing)
- [ ] Python 3.10.12 environment activates (NOT 3.11/3.12)
- [ ] `mlagents-learn --help` displays help text
- [ ] PyTorch 2.1.1 or 2.2.1 installed and importable
- [ ] Unity Package Manager shows ML Agents 2.0.1 or 2.0.2 installed
- [ ] Unity Package Manager shows Sentis 2.1.1+ installed
- [ ] Can import `using Unity.MLAgents;` in C# script without errors
- [ ] Can import `using Unity.Sentis;` in C# script without errors
- [ ] No ML-Agents or Sentis related console errors in Unity 6
- [ ] `docs/ML_AGENTS_SETUP.md` created with Unity 6 setup instructions

**Testing Instructions:**
- Create minimal test script that inherits from `Unity.MLAgents.Agent`
- Verify script compiles without errors in Unity 6
- Create minimal test that references `Unity.Sentis` namespace
- Delete test scripts after verification

**Commit Message Template:**
```
chore(ml-agents): install ML-Agents 2.0 and Sentis 2.1 for Unity 6

- Verified Unity 6.0 upgrade complete with all tests passing
- Created Python 3.10.12 virtual environment (conda recommended)
- Installed ML-Agents 2.0.x with PyTorch 2.1.1/2.2.1 dependencies
- Added Unity ML-Agents 2.0.1/2.0.2 package (Unity 6 default)
- Added Unity Sentis 2.1 package (replaces deprecated Barracuda)
- Verified Unity 6 + ML-Agents 2.0 + Sentis 2.1 compatibility
- Created ML_AGENTS_SETUP.md with Unity 6 installation instructions
- Confirmed Unity.MLAgents and Unity.Sentis namespaces available
